...
def to_checksum_addresses():
    """Konversi semua alamat ke format checksum."""
    for i in range(len(my_addresses)):
        my_addresses[i] = Web3.to_checksum_address(my_addresses[i])
    for network in networks:
        networks[network]['contract_address'] = Web3.to_checksum_address(networks[network]['contract_address'])

def get_brn_balance(web3, my_address):
    balance = web3.eth.get_balance(Web3.to_checksum_address(my_address))
...
def send_bridge_transaction(web3, account, my_address, data, network_name):
    ...
    try:
        gas_estimate = web3.eth.estimate_gas({
            'to': Web3.to_checksum_address(networks[network_name]['contract_address']),
...
def main():
    ...
    to_checksum_addresses()  # Konversi alamat sebelum mulai
